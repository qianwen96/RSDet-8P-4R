...
def train():
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        for i in range(num_gpu):
            ...
            #                                  Tout=tf.float32)
            gtboxes_and_label_r = tf.reshape(gtboxes_and_label_batch[i], [-1, 9])
                    ...
                    with tf.name_scope('tower_%d' % i):
                        with slim.arg_scope(
                            ...
                            with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane,
                                ...
                                gtboxes_and_label_h = tf.reshape(gtboxes_and_label_h, [-1, 5])
                                gtboxes_and_label_r = tf.reshape(gtboxes_and_label_r, [-1, 9])
            ...
            for step in range(cfgs.MAX_ITERATION // num_gpu):
                ...
                if step % cfgs.SHOW_TRAIN_INFO_INTE != 0 and step % cfgs.SMRY_ITER != 0:
                    ...
                    if step % cfgs.SHOW_TRAIN_INFO_INTE == 0 and step % cfgs.SMRY_ITER != 0:
                        ...
                        _, global_stepnp, total_loss_dict_,box_score_ = \
                            sess.run([train_op, global_step, total_loss_dict,outputs[0]])
